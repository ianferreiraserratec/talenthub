...
Este documento registra as regras efetivamente implementadas na Fase 1. A documentação operacional detalhada, com papéis, SLAs e exceções, será consolidada em etapa posterior.
...
1. não apto → `Inelegível`;
2. contratação `Ativa` ou `Em acompanhamento` → `Contratado`;
3. processo ativo `Bloqueado` → `Bloqueado`;
4. outro processo ativo → `Em processo`;
5. marcação manual de carência → `Carência`;
6. indisponibilidade explícita ou status manual inativo → `Inativo`;
7. caso contrário → `Disponível`.
...
O contrato de dados prevê `data_bloqueio` e `data_limite_bloqueio`. Ao implementar a transição para `Bloqueado`, o limite padrão será 30 dias, com liberação rastreável.

## Matchmaking — Fase 2

O MVP usará regras configuráveis e explicáveis, sem API externa. Critérios `Exclusivo` e `Obrigatório` eliminam; `Prioritário` e `Desejável` pontuam; `Informativo` apenas aparece na justificativa. Dados de diversidade só participarão quando declarados explicitamente na vaga.

## Shortlist e contratação — Fase 2

A geração de shortlist criará, na mesma operação protegida por lock, `TH_SHORTLISTS`, `TH_SHORTLIST_ITENS` e `TH_PROCESSOS`. A contratação criará `TH_CONTRATACOES`, atualizará o processo, recalculará o pool e registrará evento/auditoria.
